...
int xtf_evtchn_cleanup(domid_t domain_id)
{
    ...
    if ( rc != 0 )
    {
        XTF_MON_ERROR("Error disabling monitor\n");
        return rc;
    }
    ...
    if ( rc != 0 )
    {
        XTF_MON_ERROR("Failed to unbind XEN event channel\n");
        return rc;
    }
    ...
    if ( rc != 0 )
    {
        XTF_MON_ERROR("Failed to close XEN event channel\n");
        return rc;
    }
    ...
    for (;;)
    {
        ...
        while ( RING_HAS_UNCONSUMED_REQUESTS(&evt->back_ring) )
        {
            ...
            case VM_EVENT_REASON_MEM_ACCESS:
                XTF_MON_INFO("mem_access rip = %016lx gfn = %lx offset = %lx gla =%lx\n",
            ...
            case VM_EVENT_REASON_SINGLESTEP:
                XTF_MON_INFO("Singlestep: rip=%016lx, vcpu %d, altp2m %u\n",
                    req.data.regs.x86.rip,
                    req.vcpu_id,
                    req.altp2m_idx);
